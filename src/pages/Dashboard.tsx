...
import { LoadingSkeleton, SportIcon } from '@/components/ui'
      ...
      <div className="grid grid-cols-2 gap-4" data-testid="week-stats">
        {tiles.map(({ key, value, delta, fmt }) => (
          ...
          >
            ...
            <DeltaBadge delta={delta} format={fmt} />
          </div>
        ))}
      </div>
...
// ─── DeltaBadge ───────────────────────────────────────────────────────────────
//
// Week-over-week movement. More volume is rendered as positive (emerald) —
// this is a training log, not a golf score. Exact zero is flat: with additive
// totals a 0 delta means genuinely identical weeks, so no noise threshold is
// needed (unlike averaged 0–100 scores).

function DeltaBadge({ delta, format }: { delta: number; format: (n: number) => string }) {
  const { t } = useTranslation('common')
  const flat = delta === 0
  const cls = flat
  ...
  return (
    <span
      ...
      title={t('week.vsPrevWeek')}
      data-testid="week-delta"
    >
      {text}
    </span>
  )
